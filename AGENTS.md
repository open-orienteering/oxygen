...
- Eventor is stubbed, never called for real. Each stack runs `e2e/eventor-stub.mjs` and the API gets `EVENTOR_API_BASE_URL` pointing at it. Never write an E2E test that depends on `eventor.orientering.se` being reachable; add an endpoint to the stub instead (see `docs/e2e-eventor-stub.md`).
