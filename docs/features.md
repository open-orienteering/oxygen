...
The Course Editor (overflow menu) turns the map into an editing surface with contextual actions — no tool switching: click empty map and a floating menu offers **Add control** (the next free punch code ≥ 31 is suggested automatically) or **Add to course**; click an existing control for **Add to course**, **Edit description** and **Delete**; click a course leg to **Insert into course** at that spot. Drag any control to move it — if it's shared, an amber chip warns which other courses the move affects, and the selection menu shows "Also in: …". **Edit description** opens a full IOF control-description editor: symbol pickers for every sheet column (which of similar, control feature, combination, flag location) with English/Swedish tooltips, a dimensions field, and a live preview row — saved onto the control itself so it follows the control across courses. A floating panel over the map builds whole courses: create a course, click it to edit — its controls stay full-strength purple, numbered 1, 2, 3…, while everything else fades Purple Pen-style (the **Hide other controls** toggle removes them entirely), and the panel shows the control sequence with per-leg and total distances live. Reorder or remove controls per row, toggle first-as-start / last-as-finish, and undo/redo everything with `Ctrl+Z` / `Ctrl+Shift+Z` — the editor opens fitted to the controls and never auto-pans while editing. Pencil icons on the Courses and Controls pages deep-link straight into the editor with that course or control selected. Positions are stored in the map's paper-millimetre coordinate space and converted to WGS84 server-side; moving a control automatically rebuilds the straight-leg geometry and length of every course that visits it.
