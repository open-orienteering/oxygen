...
**Reference-data journaling (deferred from Step 2b into Step 3).** Step 2b journals the runner-state / punch / readout surface — the mutations that actually book race results. Class / course / control edits (`classRouter`, `course`, `control`) are *also* in the venue-owned closure during a lease, but their mutations are **not yet journaled**. This is deliberate: reference data is a mutable relational FK graph (course→course-controls ordering, class→course assignment, control codes/status, control-unit hardware telemetry that isn't race-critical), so node-portable payloads and apply cases need the seq↔UUID + codes-string design that Step 3 builds for node-to-node apply anyway. The lease-window risk is small — reference-data setup is overwhelmingly pre-race (cloud-owned) and edited during a race only for corrections (e.g. voiding a control). Until Step 3, a venue that edits reference data mid-lease must re-sync it at checkin; the checkout snapshot import (Step 4) already re-hydrates it. See `docs/offline-architecture.md` § "Reference data during a lease".
