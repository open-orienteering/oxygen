    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search name, control code..."
            value={search}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <SortHeader label="Controls" active={sort.key === "controls"} direction={sort.dir} onClick={() => toggle("controls")} className="w-24" />
                  <SortHeader label="Length" active={sort.key === "length"} direction={sort.dir} onClick={() => toggle("length")} className="w-24" />
                  <SortHeader label="Maps" active={sort.key === "maps"} direction={sort.dir} onClick={() => toggle("maps")} className="hidden md:table-cell w-20" />
                  <th className="px-4 py-2.5 text-left font-medium text-slate-500 hidden lg:table-cell w-32">Options</th>
    ...
    <div className="bg-blue-50/60 border-t border-blue-100 p-5">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. 67;39;78;53;44;50;"
            />
            <p className="text-xs text-slate-400 mt-1">
              Semicolon-separated control codes.
              {d.controlCount > 0 && (
                <span className="ml-1 font-medium text-slate-500">
                  {d.controlCount} control{d.controlCount !== 1 ? "s" : ""}
          ...
          <div className="flex gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm text-slate-600">Use first control as start</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                ...
                className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
              />
              <span className="text-sm text-slate-600">Use last control as finish</span>
            </label>
          </div>
        </div>
        ...
        <div>
          ...
          {d.classes.length === 0 ? (
            <p className="text-sm text-slate-400">Not assigned to any class</p>
          ) : (
              ...
              {d.classes.map((cls) => (
                <div key={cls.classId} className="px-4 py-2.5 flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-500">
                    {cls.runnerCount} runner{cls.runnerCount !== 1 ? "s" : ""}
                  </span>
                </div>
              ))}
              <div className="px-4 py-2 bg-slate-50 text-xs text-slate-500">
                Total: {d.classes.reduce((sum, c) => sum + c.runnerCount, 0)} runners across {d.classes.length} class{d.classes.length !== 1 ? "es" : ""}
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-blue-200 shadow-sm p-5 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-slate-900">New Course</h3>
        <button
          onClick={onClose}
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="sm:w-20">
            <label className="block text-xs font-medium text-slate-500 mb-1">Maps</label>
            <input
              type="number"
              value={numberOfMaps}
        ...
        <div>
          <label className="block text-xs font-medium text-slate-500 mb-1">Controls</label>
          <input
            type="text"
            value={controls}
            onChange={(e) => setControls(e.target.value)}
            placeholder="e.g. 67;39;78;53;44;50; (optional)"
        ...
        <div className="flex gap-5">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
            />
            <span className="text-sm text-slate-600">First as start</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              ...
              className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer"
            />
            <span className="text-sm text-slate-600">Last as finish</span>
          </label>
        </div>
