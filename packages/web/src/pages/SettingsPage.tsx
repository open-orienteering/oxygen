...
function isSettingsTab(value: string | null): value is SettingsTab {
  return (
    value !== null &&
    ([...CLUB_TABS, ...ADMIN_TABS] as readonly string[]).includes(value)
  );
}
...
export function SettingsPage() {
  ...
  // Admin-only tabs are hidden rather than disabled, so a member never
  // sees a door they cannot open. Deep links to them fall back to maps.
  const canAdmin = !authEnabled || Boolean(user?.isAdmin);
  const visibleTabs: SettingsTab[] = canAdmin
    ? [...CLUB_TABS, ...ADMIN_TABS]
    : [...CLUB_TABS];

  const [searchParams, setSearchParams] = useSearchParams();
  const requested = searchParams.get("tab");
  const tab: SettingsTab =
    isSettingsTab(requested) && visibleTabs.includes(requested)
      ? requested
      : "maps";
  const setTab = (next: SettingsTab) => {
    setSearchParams(next === "maps" ? {} : { tab: next }, { replace: true });
  };
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        <nav
          ...
          className="flex flex-wrap gap-2 border-b border-slate-200 mb-6"
        >
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <NorthLinesBadge
                        stalenessDeg={row.meridianStalenessDeg}
                        compact
                      />
                      {row.meridianStalenessDeg == null && (
                    ...
                    <div className="mt-3 flex gap-2">
                      {canManageSource(row.uploadedBy) && (
                        <button
                          ...
                          onClick={async () => {
                            const file = await utils.clubMap.download.fetch({
                              id: row.id,
                            });
