...
function dumpAll700Syms() {
    ...
    while (blockOff > 0 && blockOff < fileData.length) {
        ...
        for (let i = 0; i < 256; i++) {
            ...
            if (sym >= 700000 && sym < 800000) {
                const key = `${sym}_type${objType}`;
                symCounts.set(key, (symCounts.get(key) || 0) + 1);
            }
        }
        blockOff = nextBlock;
    }
    console.log("All symbols in 700xxx block:", Object.fromEntries(symCounts));
}
dumpAll700Syms();
